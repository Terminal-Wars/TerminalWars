...
async function drawChar(char, x, y, white=false, bold=false) {
	let mod = white+bold;
	for(let i = 0; i <= char.length; i++) {
		ctx.drawImage(charmap,8*characters.indexOf(char[i]),0+(16*mod),8,16,x+(i*8),y,8,16);
	}
}
export async function drawChars(string,x,y,white=false,bold=false,maxX=Infinity,minY=-1*Infinity,maxY=Infinity) {
	let offset = x;
	for(let i in string) {
		...
		switch(k) {
			...
			default:
				...
				await drawChar(k,x,y,white,bold);
				x += 8;
			if(x >= maxX) {x = 0; y += 12;}
		}
	}
